...
messaging.setBackgroundMessageHandler(payload => {
  ...
  self.registration.showNotification(notification.data.title || 'Calligraphy.org', notification);
});
...
if (typeof isLocal == 'undefined' || true) {
  /**
   * Cacheing
   * 
  ...
  const CACHE_NAME = environment.cacheName + `-version-${VERSION || 'NO_VERSION'}`;
  ...
  console.info('sw cacheing disabled for local development');
}
